...
class robobo_data_experiment(object):
    ...
    def check_robobo_validity(
        ...
        future_rob_box_dist = self.obtain_dist(
            exp_data.box_dx.data - (exp_data.rob_dx.data + inc_x), exp_data.box_dy.data - (exp_data.rob_dy.data + inc_y)
        )

        if (
            (exp_data.rob_dx.data + inc_x > self.area_limit_l[0])
            and (exp_data.rob_dx.data + inc_x < self.area_limit_l[1])
            and (exp_data.rob_dy.data + inc_y < self.area_limit_l[2])
            and (exp_data.rob_dy.data + inc_y > self.area_limit_l[3])
    ...
    def core_loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            while not reward:
                ...
                rob_move_msg.dest.angle.data = math.radians(
                    self.choose_action_selector(self.action_nature)(sensor_data)
                )
