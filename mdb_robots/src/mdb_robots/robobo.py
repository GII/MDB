...
class Robobo(Robot):
    """ROS node to provide Robobo sensorization to MDB."""
    ...
    def __init__(self, **kwargs):
        ...
        self.obj_det_pub = rospy.Publisher("/tracking/ball_flag", Bool, queue_size=1)
    ...
    def insert_rectangle(self, im, bbox, perimeter=True, color=[1.0, 1.0, 1.0]):
        if perimeter == True:
            rr, cc = draw.polygon_perimeter(
                [bbox[0], bbox[0], bbox[2], bbox[2]], [bbox[1], bbox[3], bbox[3], bbox[1]], shape=im.shape, clip=True
            )
        else:
            rr, cc = draw.polygon(
                np.array([bbox[0], bbox[0], bbox[2], bbox[2]]),
                np.array([bbox[1], bbox[3], bbox[3], bbox[1]]),
                shape=im.shape,
            )
        im[rr, cc, :] = color
        return im
    ...
    def detect_object(
        self, im, regions, obj, threshold=0.3, min_px_size=30, percentage=0.5
    ):  # threshold=0.15, min_px_size=30):
        rgb_obj = obj[1]
        idx_all = []
        suma = rgb_obj
        for reg in regions:
            ...
            total_pixel = len(i)
            rgb_region = np.hstack(
                [
                    np.reshape(im[i, j, 0], [np.size(i), 1]),
                    np.reshape(im[i, j, 1], [np.size(i), 1]),
                    np.reshape(im[i, j, 2], [np.size(i), 1]),
                ]
            )
        ...
        if rospy.has_param("/baxter_sense"):
            ...
            if self.do_sense:
                ...
                if self.first_iteration:
                    self.mask_table = self.segment_table(
                        im, self.table_v
                    )  # Se haria una vez, al comienzo del experimento
                ...
                for obj in self.objects:
                    ...
                    for idx in idx_all:
                        ...
                        if obj[0] == "robobo" and not robobo:
                            ...
                            if (
                                (obj[0] == "cilindro" and not cylinder)
                                or (obj[0] == "cesta" and not box)
                                or (obj[0] == "cesta2" and not box2)
                            ):
                ...
                if (
                    self.exp_rec == "mot"
                    and self.c0_val != None
                    and self.c1_val != None
                    and self.c2_val != None
                    and self.cc_val != None
                    and self.tag_detect
                ):
                    ...
                    rr, cc = draw.line(
                        self.cc_val[1],
                        self.cc_val[0],
                        (self.c1_val[1] + self.c2_val[1]) / 2,
                        (self.c1_val[0] + self.c2_val[0]) / 2,
                    )
