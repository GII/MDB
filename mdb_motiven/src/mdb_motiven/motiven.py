...
class MOTIVEN(object):
    ...
    def init_ros(self, log_level):
        ...
        self.goal_activation_topic_pb = rospy.Publisher(
            "/mdb/motiven/goal_activation", GoalActivationMsg, latch=True, queue_size=0
        )
    ...
    def executed_policy_cb(self, policy_id):
        ...
        if self.correlations_manager.correlations[self.active_corr].i_reward_assigned == 0:
            self.correlations_manager.assignRewardAssigner(
                self.active_corr, list(self.sens_t1.values()), self.active_goal
            )
        ...
        if not (self.iterations % 100):
            yaml.dump(
                self, open(file_name, "w", encoding="utf-8"), Dumper=yamlloader.ordereddict.CDumper, allow_unicode=True
            )
    ...
    def run_standalone(self):
        ...
        while not self.stop and not rospy.is_shutdown():
            ...
            self.motivation_pb.publish(
                String(
                    "Motivation: "
                    + str(self.active_mot)
                    + "\nActive SUR: "
                    + str(self.active_corr + 1)
                    + ", Correlated Sensor: "
                    + str(self.corr_sensor)
                    + " "
                    + str(self.corr_type)
                )
            )
            ...
            if self.correlations_manager.correlations[self.active_corr].i_reward_assigned == 0:
                self.correlations_manager.assignRewardAssigner(
                    self.active_corr, self.episode.getSensorialStateT1(), self.active_goal
                )
    ...
    def motivation_manager_ltm(self):
        ...
        if self.use_motiv_manager:
            if self.correlations_manager.correlations[self.active_corr].goal != self.active_goal:
                self.active_corr = self.correlations_manager.getActiveCorrelationPrueba(
                    list(self.sens_t1.values()), self.active_goal
                )
            self.corr_sensor, self.corr_type = self.correlations_manager.getActiveCorrelation(
                tuple(self.sens_t1.values()), self.active_corr, self.active_goal
            )
            if self.corr_sensor == 0:
                self.active_mot = "Int"
            else:
    ...
    def memory_manager(self):
        ...
        if self.active_mot == "Int":
            ...
            if self.episode.getReward():
                ...
                if self.correlations_manager.correlations[self.active_corr].i_reward_assigned == 0:
                    self.correlations_manager.assignRewardAssigner(
                        self.active_corr, self.episode.getSensorialStateT1(), self.active_goal, 1
                    )
                ###
                ...
                self.active_corr, self.reward, tuple(self.episode.getSensorialStateT1()), self.active_goal
            ):
                self.correlations_manager.correlations[self.active_corr].correlationEvaluator(
                    self.traces_buffer.getTrace()
                )
    ...
    def plot_graphs(self):
        ...
        ax2.plot(
            list(range(n_reward)), iter_goal, marker=".", markersize=1.0, color="green", linewidth=1.0, label="active"
        )
    ...
    def object_in_close_box(perceptions):
        ...
        if not LTMSim.object_too_far(perceptions["box1_dist"], perceptions["box1_ang"]):
            inside = (abs(perceptions["box1_dist"] - perceptions["ball_dist"]) < 0.05) and (
        ...
        if not inside:
            if not LTMSim.object_too_far(perceptions["box2_dist"], perceptions["box2_ang"]):
                inside = (abs(perceptions["box2_dist"] - perceptions["ball_dist"]) < 0.05) and (
    ...
    def object_with_robot(perceptions):
        ...
        if (not perceptions["ball_in_left_hand"]) and (not perceptions["ball_in_right_hand"]):
            ...
            together = (abs(perceptions["ball_dist"] - dist_near) < 0.05) and (
    ...
    def get_final_state(cls, perceptions_t1, perceptions_t):
        ...
        if (None in list(perceptions_t.values())) or (None in list(perceptions_t1.values())):
            ...
            if cls.object_with_robot(perceptions_t1):
                ...
                elif not LTMSim.object_too_far(
                    perceptions_t1["ball_dist"], perceptions_t1["ball_ang"]
                ) and LTMSim.object_too_far(perceptions_t["ball_dist"], perceptions_t["ball_ang"]):
    ...
    def update_reward_values(self, Trace):
        ...
        for trace in Trace:
            self.reward_dict[trace[0]].mean_value = (
                self.reward_dict[trace[0]].mean_value * self.reward_dict[trace[0]].n_values + trace[1]
