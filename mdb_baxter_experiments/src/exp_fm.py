...
from mdb_baxter_policies.srv import BaxThrow, BaxP, BaxGB, BaxDB, BaxG, PickAdj, BaxRAP, BaxCF, GetSense, BCheckR, BCheckRRequest, BaxSense
...
from mdb_common.srv import BaxChange, ExecPolicy, NewExperiment, RefreshWorld
...
class exp_fm():
	...
	def execute_policy(self, policy_code):
		...
		if policy_code == 'sweep_object':
			...
			srv.obj_sens.height.data = -0.035+self.choose_sweep_height(self.obj_type)
			#Dest destination
			...
			srv.dest_sens.height.data = -0.035+self.choose_sweep_height(self.obj_type)
			#Rest of the service request
